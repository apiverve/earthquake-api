...
class EarthquakeValidationException implements Exception {
  final List<String> errors;

  EarthquakeValidationException(this.errors);

  @override
  String toString() => 'EarthquakeValidationException: ${errors.join("; ")}';
}
...
/// Worldwide Earthquakes API client.
///
/// For more information, visit: https://apiverve.com/marketplace/earthquake?utm_source=dart&utm_medium=readme
///
/// Parameters:
...
class EarthquakeClient {
  ...
  EarthquakeClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/earthquake',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [EarthquakeValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw EarthquakeValidationException(errors);
    }
  }

  /// Execute a request to the Worldwide Earthquakes API.
  ///
  /// Parameters are validated before sending the request.
  Future<EarthquakeResponse> execute() async {
    if (apiKey.isEmpty) {
      throw EarthquakeException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw EarthquakeException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw EarthquakeException('Resource not found');
      } else {
        throw EarthquakeException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is EarthquakeException) rethrow;
      throw EarthquakeException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Worldwide Earthquakes API client.
